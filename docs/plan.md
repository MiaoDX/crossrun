...
```
┌──────────────────────────────────────────────────────────┐
│  模型容器层：每模型一个容器（不进 repo）                     │
│  统一暴露 4 个操作：观测更新 / 动作预测 / policy reset / 批量 │
│  ┌──────────┐ ┌──────────┐ ┌──────────────┐ ┌─────────┐  │
│  │ π0.5     │ │ OpenVLA  │ │ XPolicyLab   │ │ 自研/WAM│  │
│  │          │ │          │ │ (内含 40+)   │ │         │  │
│  └────┬─────┘ └────┬─────┘ └──────┬───────┘ └────┬────┘  │
└───────┼────────────┼──────────────┼──────────────┼───────┘
        └────────────┴───────┬──────┴──────────────┘
...
**XPolicyLab 的归位**：它的四操作接口（观测更新 / 动作预测 / policy reset / 批量查询）被采纳为**所有模型容器的统一契约**；XPolicyLab 本身则是"一个恰好内含 40+ 模型的容器"。两个角色不冲突，也不能混为一谈 —— **它是策略服务框架，不是一个模型。**
